...
import time as t # Used to delay the request timing
...
def crack(passsel, passlist, website):
    ...
    try: # Check if password field css selector is valid
        ...
        Sel_pass = browser.find_element_by_id(passsel) # txtPAN
    ...
    try: # Start the attack
        for password in f: # Run the attack untill the password list is over
            ...
            # Sel_user = browser.find_element_by_css_selector(usersel)
            # Sel_pass = browser.find_element_by_css_selector(passsel)
            Sel_pass = browser.find_element_by_id(passsel)
...
if options.website == None and options.passsel == None and options.passlist == None: # if no arguments are given0
    wizard()
missing_args = ""
